...
Step Functions Regras de Negócio
  ↓
Lambda Criar Processo para Execução Regras Negocio
  ↓
INPUT_LOCK
  ↓
...
- Itens devem obrigatoriamente ter `CURRENT_SOR` ou `input_overrides`.
...
# 6. Lambda Criar Processo para Execução das Regras de Negócio

## Nome sugerido

```text
lambda-criar-processo-regras-negocio
```
...
```text
sfn-orq-seguros-regras-negocio
```
...
2. Obter `data_referencia` do `PROCESS METADATA`.
3. Gravar SPEC com:
   - `process_id`
   - `data_referencia`
